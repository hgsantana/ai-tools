...
- Saved under `plans/` in the working repository. A plan's whole set — base plan, stage files, fix files — stays there for the entire execution and moves to `plans/finished/<slug>/` in a single move, only once **every stage has finished (`F`)**.
- An archived set is never picked up on its own. One still holding a stage that failed for good — which happens only when archiving was chosen anyway, below — returns to `plans/` only when the orchestrator is dispatched on it by name; one whose stages all finished is final, and the attempt is refused.
...
- **The archival commit belongs in the pull request the work opens.** Archive before opening it, or add the commit to one already open, so the reviewer sees the plan files being removed and can read them against the change they describe. That review is what makes the deletion safe: the plan is checked against the work in the one place both are visible together.
- **A stage that failed for good (`E`) stops the archival — nothing moves, not even the finished stages.** Report that the plan ended with N failed stages and ask which the user wants: retry the failed stages, open the pull request and archive anyway, or open the pull request and leave the whole set in `plans/`. Return that question the way a push or a pull request is returned — as an approval item to whoever dispatched the run, for the coordinator to relay — and never decide it silently. The one exception is an unattended Vibe Coding delivery, whose gate has already promised no further checkpoints: it decides for itself and records the choice, with its reasoning, in its decisions file.
