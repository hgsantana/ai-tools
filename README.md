...
24. `dev-ai-tools` Task mode agrees one task with the user, writes `dev/<slug>.md`, and implements that single stage in the session. Specified and queue modes spawn a fresh session-model execution pass per plan: the pass records the unit on `plan/<slug>`, implements each stage, sends tests to a default worker, archives by copy-then-remove, and opens a pull request targeting the recorded base. The branch history is symmetric: the first commit introduces the plan or task file, and the last removes it. `vibe-ai-tools` plans with the user, then spawns a fresh execution pass that uses implementers for stage code. `campaign-ai-tools` repeatedly invokes planning and execution passes under user-defined priorities: each pass receives its job in the spawn payload and does not read skill files; the host stores only their short signals; accepted commits accumulate locally on `improve/<campaign>` without a push or pull request.
