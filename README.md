...
7. Skills are harness-agnostic — no per-harness copies. One file: `skills/<name>/SKILL.md` (frontmatter per rule 9, then the whole skill). No skill contract, no skill base, no `skills/<name>.md`, no `skills/SKILL-CONTRACT.md`, no `skills/MAINTAINER.md`. Duplicated maintainer text is required, not drift. `description` two parts, at most 500 characters (rule 9). No character cap on the skill body; rule 15 still forbids fluff. No skill contains a **Stake** heading or **Continue?**. `USER-AGENTS.md` routing is the one gate: each named skill's `description` `Impact:` from memory (no file opened for it) and that skill's **Min. role** against this harness's `$HOME/.ai-tools/MODELS.md` row (that cell's model token and every higher-role cell to its left), then one question (a skill, **run it here**, or **stop**). The gate is not used for anything except activating a shipped `*-ai-tools` skill.
8. **Spawn depth is one**: only the user's session spawns agents, and a spawned agent spawns nothing — it returns the work it cannot carry as a dispatch request (`agents/SUBAGENT-CONTRACT.md`). Harnesses that do allow a deeper chain gain nothing from it; the design assumes the shallowest. Dispatch of code-writing work is serial — one such agent runs at a time; read-only discovery, builds, and tests may run concurrently. The **planner** role orchestrates, so it cannot be dispatched — a dispatched planner would have no way to spawn its workers. Dispatched work goes to `implementer-ai-tools` or `mechanical-ai-tools`, whose wrappers pin the model. `USER-AGENTS.md` routing gates every shipped skill: naming a skill is the yes, then this session follows that file's Workflow at its **Min. role** (planner-min skills carry `planner-ai-tools`). The gate never refuses over the session model; proceeding below the minimum is the user's call.
9. Skill frontmatter: only universally accepted keys (`name`, `description`) plus optional keys every supported harness tolerates (e.g. `argument-hint`). A key any supported harness rejects does not belong in a shared file. `description` is what the harness keeps in context without opening the skill body, so it states two things, in this order: (1) what the skill does, briefly, including when to use it (`/name` and the situations that should activate it); (2) `Impact:` plus the Stake — what can be billed, deleted, committed, pushed, or otherwise changed, or that there is none. Cap **500 characters**: harnesses budget the skill *list*, not the body — Codex caps it at 2% of the context window or 8,000 characters, Claude Code truncates a description at 1,536.
...
5. **Skills** — link each `skills/*-ai-tools` directory into each scoped skills root; the same shared directory serves every harness (rules 7–9). Prefer these links over harness scan paths (Grok `[skills] paths`), which can clobber existing names.
...
7. **Verify** — instructions resolve into the clone, `USER-AGENTS.md` fits the 12,000-character cap (rule 3), `agents/SUBAGENT-CONTRACT.md` and every agent base exist at the pinned path, every shipped `skills/<name>/SKILL.md` exists, and every installed agent and skill is a link or an unmodified copy. Skipped under `--dry-run`; re-run anytime with `verify`.
