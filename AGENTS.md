...
|----------|----------------|------------------------------------------|
| **planner** | High-reasoning work: decompose work, design approaches, own acceptance judgment, validate deliveries, decide whether to retry or escalate. Does **not** write production code while orchestrating `/dev-ai-tools`. | planning agent, thinking model, orchestrator |
...
2. The **current session** is usually the **planner** when running `/plan-ai-tools` or orchestrating `/dev-ai-tools`.
...
3. For non-trivial code change requests, use `/plan-ai-tools` to draft and save the plan. That skill owns planning detail (source of truth, per-type tests, docs, commit boundaries, execution graph, multi-file stage layout, `.gitignore`). Do not restate that machinery here.
4. **ALWAYS ASK** whether to implement the plan now or leave it documented. If the user chooses to leave it, stop.
5. If the user chooses to implement, use `/dev-ai-tools` (bare form) to execute. Warn first: bare `/dev-ai-tools` processes **every** plan currently at the root of `plans/*.md` (base plan files), not only the one just created.
6. `/dev-ai-tools` owns implementation, validation, correction rounds, status updates on the plan table, and moving finished stage/base files to `plans/finished/`. Do not re-validate its work or duplicate its logic outside the skill.

## Tools

Use the `/az-ai-tools` skill for Azure resources with the Azure CLI.

Use the `/gh-ai-tools` skill for GitHub resources with the GitHub CLI.

Use the `/gc-ai-tools` skill for Google Cloud resources with the Google Cloud CLI.
