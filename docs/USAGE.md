...
The `<skill_offer>` inside `<routing_gate>` is the only `USER-AGENTS.md` gate. It names each option's impact and that choosing it dispatches the agent named in `Agent:`, then offers the relevant skill, **run it here**, and **something else**. The last choice lets the user name another skill, revise the request, or propose a different approach; a native **Other** field serves the same purpose. A leading `/name` confirms that skill's stake, offers other fitting skills, and proceeds after the answer. Choosing a skill runs its `<session_workflow>`, which coordinates delivery and delegates compute-heavy tasks to the worker named in each cited `<template>`'s `agent` attribute, via populated payloads from `<dispatch_templates>`. A workflow that later invokes another skill does not re-enter `<routing_gate>`. Later approvals still follow the skill's own rules.
...
Agents are not user-facing skills. Do not offer or invoke them as alternatives to a slash skill; the session workflow spawns the agent named in the cited `<template>`'s `agent` attribute, passing its populated payload from `<dispatch_templates>`.
