...
- Generate the patch with `git diff <default>...<branch> --output=dev/wip/<slug>-review.patch` — git writes the file directly. Verify it only via `--stat` or `wc -l`. Opening diffs in an editor is the relaying session's concern, not yours: you produce only universal artifacts (branch, patch, stat). Never produce the patch with a file-writing tool, and leave its content on disk.
...
- Scope is the active plan. Leave unrelated plans and `dev/wip/**` unread.
...
A plan's **set** is its base plan `dev/<slug>.md` plus every `dev/<slug>-*.md` stage and fix file. The set is one unit and travels as one: it is created under `dev/` and every file of it stays there, whatever its status, until the plan is over. Move files of a plan only when every stage is terminal (`F` or `E`).
...
- **The queue reads work under `dev/`.** `dev/finished/**` stays out of Mode A step 2. This is what makes spontaneous re-execution impossible by construction.
- An archived set is picked up only on a dispatch naming it by slug or path, and resolves to exactly `dev/finished/<slug>/<slug>.md`; anything else is not an archived set — report it and leave it. It holds an `E` stage only when archiving anyway was the chosen answer above. Read that base plan's stage table:
  ...
  - every stage `F` → the set is final: leave it, and report the refusal. Refuse the same way when `dev/<slug>.md` already exists — one set per slug.
...
2. Discover base plans: `dev/*.md` (excluding `*-<digits>.md` stage files, `*-F<digits>.md` fix files, `dev/finished/**`, and `dev/wip/**`). Execute a stage or fix only with its base.
