...
Executing accepted plans under `dev/`, or an explicit ad-hoc brief, unattended. That work is defined by `$HOME/.ai-tools/agents/dev-ai-tools.md` (Windows: `%USERPROFILE%\.ai-tools\agents\dev-ai-tools.md`). This skill only decides **who runs it**: the shipped `dev-ai-tools` agent, on the model its wrapper pins, or this session, on the model it already has. Never implement outside one of those two routes.

## Agent and category

Agent: `dev-ai-tools`, base `$HOME/.ai-tools/agents/dev-ai-tools.md` (Windows: `%USERPROFILE%\.ai-tools\agents\dev-ai-tools.md`). Category for the contract's model check: **planner**.
...
Announce it in chat, then read `$HOME/.ai-tools/agents/dev-ai-tools.md` in full and follow it as your own rule set for this request. It is the absolute rule set; this skill adds only what follows.
