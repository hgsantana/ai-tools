...
1. Read `$HOME/.ai-tools/MODELS.md` (Windows: `%USERPROFILE%\.ai-tools\MODELS.md`) and take the row of the harness you are running in. The dispatched agent's model is that row's column for the category the base names (**planner** / **implementer** / **mechanical**); compare model tokens only — a `· effort` note in the cell is advisory.
...
- **Dispatch the agent** — spawn the agent the base names, on the model its wrapper pins, in its own context. This session stays clean and relays every question and approval; the agent cannot talk to the user directly. The agent follows this skill's **Workflow**.
- **Stop** — nothing is read, run, or changed.

On a mismatch, that same message also states which model is running, which one this agent expects, that the current model is not the best fit for this task, and how to switch it — the row's *Change the session model* column. If this session cannot spawn agents, say so there: only **stop** remains — never run the workflow here.
