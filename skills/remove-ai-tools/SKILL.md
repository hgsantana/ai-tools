...
Remove an ai-tools installation with task `remove`.
...
1. **Scope** — ask which harnesses, instructions, and purge options are in scope. Pass the answer as `--harnesses`; an explicit "all" selects every detected harness.
