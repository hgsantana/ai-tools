...
<skill name="update-ai-tools">
  ...
  <session_workflow>
    ...
    <step id="3" name="execution">
      Execute `scripts/shell/update.sh` with exactly the approved flags as {APPROVED_FLAGS}, directly in session or by dispatching `<template role="script-runner">` from `<dispatch_templates>`.
  ...
  <dispatch_templates>
    <template role="script-runner" agent="mechanical-ai-tools">
      <job>Mechanical worker: execute the update shell script and capture output.</job>
      <input>
        <script>scripts/shell/update.sh</script>
        <flags>{APPROVED_FLAGS}</flags>
      </input>
      <instructions>
        Run scripts/shell/update.sh with {APPROVED_FLAGS}.
