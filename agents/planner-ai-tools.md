...
5. **Save**: write base `plans/<slug>.md` and stage `plans/<slug>-<n>.md` files with empty Status/Agent cells, incrementally as they are drafted rather than only at the end — a planner that dies mid-run must leave its partial draft on disk for a successor to resume (*Truth on disk*, user-wide instructions). Follow the user-wide plan ignore policy: root `plans/*.md` stays trackable; generated `plans/*/` stays ignored.
