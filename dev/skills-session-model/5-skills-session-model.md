...
- Replaced `skills/campaign-ai-tools/SKILL.md` with the target text verbatim, substituting `(current body, verbatim)` in `<return_protocol>` with the current file's five `<signal>` lines, and the three `(current text)` placeholders in `<boundaries>` with the current `strictly-local`, `preserve-history`, and `tmp-untracked` rule text. No other file touched.
- `scripts/lint.sh; echo $?` → exit `0` (`326 ok, 1 skipped, 0 warnings`; the skip is the version-bump check without `--base`). Output includes `ok: template placeholders match their input` for campaign, `ok: skill description within cap: ... (404/500)`, and resolved references to `plan-ai-tools <template role="repo-discovery">`, `dev-ai-tools <template role="stage-verifier">`, `dev-ai-tools <step id="3">`, `<template executor="session-subagent">`, and `<template executor="implementer">`.
- `git grep -n 'template role=' -- skills/campaign-ai-tools` lists exactly `campaign-planner`, `campaign-executor`, `stage-implementer` (plus their prose backtick references) — no `stage-verifier` template remains in campaign.
- `git diff` on the real `<return_protocol>` block (isolated with `awk '/^  <return_protocol>$/,/^  <\/return_protocol>$/'`, since the bare tag also appears in backticked prose elsewhere in the file) is byte-identical to the pre-edit block. The real `<step id="5">` block (isolated the same way) differs only by "At completion or controlled stop:" replacing "At controlled stop or completion:", and the two added BLOCKED/report lines — matching the Tests description exactly.
- No-fallback probe: `git grep -niE 'carr(y|ies) (that|the) work itself|records? the fallback|fall(s)? back to the session|run the payload in the session|on the session model instead' -- skills/campaign-ai-tools` → exit `1`, prints nothing.
- Positive probe: `git grep -n 'nested-spawn capability' -- skills/campaign-ai-tools` → 3 lines (rule `no-nested-fallback`, one constraint in `campaign-planner`, one constraint in `campaign-executor`).
- `scripts/test.sh` → exit `0` (`305 ok, 0 skipped, 0 warnings`).
- `shellcheck -x -P scripts/shell -P scripts/test scripts/shell/*.sh scripts/*.sh scripts/test/*.sh` → only the known `SC1071` on `scripts/shell/install-zsh.sh`.
- **Discrepancy found, not fixed (plan text, not this stage's scope):** the literal Tests-section probe `git grep -niE '(planner|implementer|mechanical)-ai-tools|agent="|coordinator|sub-dispatch|high-reasoning|\bagents\b' -- skills/campaign-ai-tools` does **not** print nothing — it matches 2 lines: `USER-AGENTS` in step 1 (the `<user_interaction>` reference) and `AGENTS.md if present` in the `stage-implementer` instructions. Both strings are mandated verbatim by this stage's target block (identical wording already lands in the committed `skills/vibe-ai-tools/SKILL.md`). The repo-wide final-acceptance grep in the base plan (`0-skills-session-model.md`) filters these two forms out with `sed -E 's/(USER-)?AGENTS(\.md)?//g; s/Agent://g'` before re-checking; this stage's literal probe has no such filter. Implemented the target text exactly as specified rather than deviating from the mandated verbatim wording to satisfy an unfiltered probe.

Coordinator acceptance note: the stage's unfiltered `\bagents\b` probe matches `USER-AGENTS` and `AGENTS.md if present`, both mandated verbatim by the target text. Accepted using the base plan's final-grep filter (`sed -E 's/(USER-)?AGENTS(\.md)?//g; s/Agent://g'`), which prints nothing. `<return_protocol>` unchanged per `git diff` (no hunk inside the block). Verifier log: dev/tmp/skills-session-model-stage5-output.log.
