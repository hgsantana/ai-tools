...
- Applied all steps to the five files: `skills/az-ai-tools/SKILL.md`, `skills/gc-ai-tools/SKILL.md`, `skills/gh-ai-tools/SKILL.md`, `skills/update-ai-tools/SKILL.md`, `skills/remove-ai-tools/SKILL.md`.
  - Descriptions re-folded and now end `Agent: session.` per file (az/gc/gh exactly matching the stage's specified trailing fragments).
  - az and gc `<overview>` second line changed to "Session handles user approvals and mutation guardrails, sending bulk CLI collection to a default worker."; gh `<overview>` left unchanged.
  - az/gc `<step id="2">` and gh `<step id="2">` now read "Send bulk log or fact collection to `<template role="mechanical-discovery">` from `<dispatch_templates>`, substituting {COMMANDS} and {TOPIC}." (gh keeps "bulk fact collection").
  - `mechanical-discovery` templates in az/gc/gh: `agent="mechanical-ai-tools"` replaced with `executor="default-worker"`; `<job>` reworded "Default worker: ...".
  - update-ai-tools: steps 2/3 rewritten to spawn `<template role="script-runner">` with `{FLAGS}`/`{LOG_PATH}`; step 4 wording changed to "reminder to restart harnesses that cache skills"; template rewritten verbatim per stage text with `executor="default-worker"`, `{FLAGS}`, `{LOG_PATH}`, and a `<constraints>` block.
  - remove-ai-tools: description rewritten verbatim (drops "remove agents", ends `Agent: session.`); steps 2/3 follow the update pattern with `dev/tmp/remove-dry-run.log` / `dev/tmp/remove-execution.log`; template follows the update pattern with `scripts/shell/remove.sh` and job "Default worker: execute the remove shell script and capture output."; step 4 left unchanged (no agent/skill wording to fix).
  - Appended the shared `default-worker` boundary rule verbatim as the last `<rule>` in `<boundaries>` of all five files.
- Tests run from `/home/wsl/.ai-tools`:
  - `scripts/lint.sh; echo $?` → exit 0, `318 ok, 1 skipped, 0 warnings`. Output includes `ok: reference resolves: <template executor="default-worker">` for each of the five files, and `ok: template placeholders match their input` for update and remove.
  - `git grep -nE '(planner|implementer|mechanical)-ai-tools|agent="|APPROVED_FLAGS' -- skills/az-ai-tools skills/gc-ai-tools skills/gh-ai-tools skills/update-ai-tools skills/remove-ai-tools` → prints nothing (exit 1, no matches). A plain working-tree `grep -r` with the same pattern also printed nothing.
  - `git grep -n 'executor="default-worker"' -- skills/az-ai-tools skills/gc-ai-tools skills/gh-ai-tools skills/update-ai-tools skills/remove-ai-tools` → exactly 10 lines: 5 template lines + 5 rule lines, one pair per file.
  - All five descriptions verified ending `Agent: session.` by reading the frontmatter.
  - `scripts/test.sh; echo $?` → exit 0, `305 ok, 0 skipped, 0 warnings`.
  - `shellcheck -x -P scripts/shell -P scripts/test scripts/shell/*.sh scripts/*.sh scripts/test/*.sh` → only the known `SC1071` on `scripts/shell/install-zsh.sh` (exit 1 from that one finding, no new finding).
- No conflicts observed with the concurrent stage 1 (models-ai-tools already removed, lint/test baselines already reflect it) or stage 3 (dev-ai-tools and plan-ai-tools already carry `executor="default-worker"` on their own templates; not touched by this stage).
- Re-verification pass (second implementer instance): found all five files already fully edited per the Steps above, including `skills/gh-ai-tools/SKILL.md`, `skills/update-ai-tools/SKILL.md`, and `skills/remove-ai-tools/SKILL.md`, contrary to a stale note describing them as untouched. Made no further edits. Re-ran the Tests: `scripts/lint.sh; echo $?` → exit 0 (`318 ok, 1 skipped, 0 warnings`); `grep -rnE '(planner|implementer|mechanical)-ai-tools|agent="|APPROVED_FLAGS' skills/{az,gc,gh,update,remove}-ai-tools` → no output; `grep -rnE 'executor="default-worker"' skills/{az,gc,gh,update,remove}-ai-tools` → exactly 10 lines (5 template + 5 rule); all five descriptions confirmed ending `Agent: session.`; `scripts/test.sh; echo $?` → exit 0 (`305 ok, 0 skipped, 0 warnings`); `shellcheck -x -P scripts/shell -P scripts/test scripts/shell/*.sh scripts/*.sh scripts/test/*.sh` → only the known `SC1071` on `scripts/shell/install-zsh.sh`.
